...
export default function Admin() {
  ...
  const loadLogs = async () => {
    try {
      ...
      // Si no hay logs, no mostrar error, solo mostrar array vacío
      if (!response.logs || response.logs.length === 0) {
        console.log('No se encontraron logs en el sistema');
      }
    } catch (error: any) {
      console.error('Error al cargar logs:', error);
      console.error('Error completo:', error.response?.data || error.message);
      setLogs([]); // Asegurar que el array esté vacío en caso de error
      setFeedback({ 
        type: 'error', 
        message: error.response?.data?.message || error.message || 'Error al cargar logs del sistema' 
      });
    } finally {
                    ...
                    {filteredLogs.length === 0 && !logsLoading && (
                      <div className="text-center py-8">
                        ...
                        <p className="text-slate-500">
                          {logs.length === 0 
                            ? 'No hay logs en el sistema todavía' 
